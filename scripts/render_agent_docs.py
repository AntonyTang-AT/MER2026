#!/usr/bin/env python3
"""生成各工作区 docs/AGENT.md（UTF-8，内容除工作区路径外完全一致）。"""
...
def get_repo() -> Path:
    ...
    while p != p.parent:
        if all((p / w).is_dir() for w in WORKSPACES):
            return p
        p = p.parent
    raise RuntimeError("Cannot locate MER2026 repo root (need main/tmx/yxp/zzj/cyx dirs)")


REPO = get_repo()


def _forbidden(member: str) -> str:
    others = [w for w in WORKSPACES if w != member]
    return "、".join(f"MER2026/{w}" for w in others)


def _shared_body() -> str:
    return """## 当前进度快照（2026-07-07）
...
def render(member: str) -> str:
    ws_path = (REPO / member).as_posix()
    forbidden = _forbidden(member)
    body = _shared_body()

    return f"""# Agent 提示文档 — 工作区 `{member}`

> **用法：** 在 Cursor 中打开 `{ws_path}` 作为工作区根目录，复制下方「一键 Prompt」作为 Agent **第一条消息**。
>
...
- 当前工作区：{member}
- 根目录：{ws_path}
- 只允许在 `{member}/` 内创建、修改、删除代码与配置
- 禁止修改 {forbidden} 等其他工作区
- 共享 data/、models/、third_party/ 为软链接（指向仓库根），勿删除共享数据
- Git 只 commit 本目录 `{member}/` 下的变更
...
## 开发前
1. cd {ws_path} && export PYTHONPATH=$(pwd):${{PYTHONPATH:-}}
...
def main() -> None:
    for member in WORKSPACES:
        out = REPO / member / "docs" / "AGENT.md"
        out.parent.mkdir(parents=True, exist_ok=True)
        out.write_text(render(member), encoding="utf-8")
        print(f"Wrote {out}")
