...
> **用法：** 在 Cursor 中打开 `/root/autodl-tmp/MER2026/main` 作为工作区根目录，复制下方「一键 Prompt」作为 Agent **第一条消息**。
>
...
## 工作区身份（必须遵守）
- 当前工作区：main
- 根目录：/root/autodl-tmp/MER2026/main
- 只允许在 `main/` 内创建、修改、删除代码与配置
- 禁止修改 MER2026/tmx、MER2026/yxp、MER2026/zzj、MER2026/cyx 等其他工作区
- 共享 data/、models/、third_party/ 为软链接（指向仓库根），勿删除共享数据
- Git 只 commit 本目录 `main/` 下的变更
...
## 开发前
1. cd /root/autodl-tmp/MER2026/main && export PYTHONPATH=$(pwd):${PYTHONPATH:-}
